...
def gen_new_map(width, height):
    box_heigth = 3
    box_width = 3

    # Create template string (empty room with a box in the middle)
    template_str = ""
    template_str += ''.join(["|" for _ in range(int(width / 2))]) + '\n'
    for y in range(int(height / 2) - 2):
        if y < int(height / 2 - box_heigth):
            template_str += ''.join(["|"] + ["." for _ in range(int(width / 2) - 1)]) + '\n'
        else:
            template_str += ''.join(["|"] + ["." for _ in range(int(width / 2) - 1 - box_width)] + ["|" for _ in range(box_width)]) + '\n'
    template_str += ''.join(["|" for _ in range(int(width / 2))])

    # initial empty map with standard ghost house
    tileMap = Map(int(width / 2), int(height / 2), template_str)
    ...
    # reflect the first 14 columns to print the map while parsing it to bool
    bool_matrix = []
    for line in str(tileMap).splitlines():
        ...
        bool_line = bool_line + bool_line[::-1]
    ...
    reversed_bool_matrix = []
    for bool_line in bool_matrix:
        bool_line.reverse()
        reversed_bool_matrix.append(bool_line)
